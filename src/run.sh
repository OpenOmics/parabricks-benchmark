...
function submit(){
  ...
  case "$executor" in
    slurm)
          ...
          CLUSTER_OPTS="sbatch --gres {params.gres} --cpus-per-task {rule.threads} -p {params.partition} -t {params.time} --mem {params.mem} --job-name={params.rname} -e $SLURM_DIR/slurm-%j_{params.rname}.out -o $SLURM_DIR/slurm-%j_{params.rname}.out"
          ...
          if [[ ${6#\'} != /lscratch* ]]; then
            CLUSTER_OPTS="sbatch --cpus-per-task {rule.threads} -p {params.partition} -t {params.time} --mem {params.mem} --job-name={params.rname} -e $SLURM_DIR/slurm-%j_{params.rname}.out -o $SLURM_DIR/slurm-%j_{params.rname}.out"
          fi
